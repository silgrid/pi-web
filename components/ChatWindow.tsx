...
import type { AgentMessage, AssistantContentBlock, AssistantMessage, BackgroundTasksClientEvent, BashExecutionMessage, BlockingExtensionUiRequest, ExtensionUiRequest, SessionInfo, SessionTreeNode, ToolResultMessage, UserMessage } from "@/lib/types";
...
interface Props {
  ...
  /** Signal from AppShell that the session with this id was written by
   *  another pi process; a new `key` triggers one disk reload. */
  externalSessionChange?: { sessionId: string; key: number } | null;
...
export function ChatWindow({ session, searchTarget, onSearchTargetHandled, initialScrollPosition, onScrollPositionChange, sessionRunning, isActivePane = true, newSessionCwd, newSessionDraftKey, onAgentEnd, onAttentionNeeded, onBackgroundTasksEvent, onSessionCreated, onSessionForked, modelsRefreshKey, chatInputRef, onBranchDataChange, onSystemPromptChange, onSystemToolsChange, onSystemInfoLoaderChange, onSessionStatsChange, onSessionStatsPanelOpen, onContextUsageChange, externalSessionChange, onOpenFile, onOpenSession, onAskInNewChat, quoteSelectionEnabled = false, initialPrompt, onInitialPromptConsumed, soundEnabled = true, onSoundToggle, playDoneSound = () => {}, unlockAudio }: Props) {
  const { t } = useI18n();
  ...
  // External-write signal: each new key reloads the mounted session from disk
  // exactly once, and only when it targets the session this window shows.
  const externalSessionChangeKeyRef = useRef<number | null>(null);
  useEffect(() => {
    if (!externalSessionChange) return;
    if (externalSessionChangeKeyRef.current === externalSessionChange.key) return;
    externalSessionChangeKeyRef.current = externalSessionChange.key;
    if (externalSessionChange.sessionId !== session?.id) return;
    void refreshFromDisk(externalSessionChange.sessionId);
  }, [externalSessionChange, session?.id, refreshFromDisk]);
  ...
  const chatInputElement = (
    <ChatInput
      ref={chatInputRef}
      onSend={isActivePane ? handleSend : (() => {}) as () => void}
      onAbort={isActivePane ? handleAbort : (() => {}) as () => void}
      onSteer={isActivePane && agentRunning ? handleSteer : undefined}
      onFollowUp={isActivePane && agentRunning ? handleFollowUp : undefined}
      onPromptWithStreamingBehavior={isActivePane && agentRunning ? handlePromptWithStreamingBehavior : undefined}
